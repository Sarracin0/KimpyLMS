...
export default function FlashcardEditor({ deck }: FlashcardEditorProps) {
  ...
  const handlePersistDeck = async (override?: Partial<{ title: string; description: string | null; isPublished: boolean }>) => {
    try {
      ...
      toast.error('Impossibile salvare le modifiche al deck')
  ...
  const handleTogglePublish = async () => {
    try {
      ...
      toast.error('Impossibile aggiornare lo stato di pubblicazione')
  ...
  const handleCreateCard = async () => {
    if (!newCard.front.trim() || !newCard.back.trim()) {
      toast.error('Aggiungi testo su fronte e retro')
      return
    }
    ...
    try {
      ...
      toast.error('Impossibile aggiungere la carta')
  ...
  const handleSaveCard = async (cardId: string) => {
    ...
    try {
      ...
      toast.error('Impossibile salvare la carta')
  ...
  const handleDeleteCard = async (cardId: string) => {
    try {
      ...
      toast.error('Impossibile eliminare la carta')
  ...
  const handleMoveCard = async (cardId: string, direction: 'up' | 'down') => {
    ...
    try {
      ...
      toast.error('Impossibile riordinare la carta')
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            {deckState.autoGenerated && (
              <Badge variant="secondary" className="gap-1 text-xs">
                <Sparkles className="h-3 w-3" /> Generato dall&apos;AI
              </Badge>
            )}
            <Badge variant={deckState.isPublished ? 'default' : 'outline'} className="text-xs">
              {deckState.isPublished ? 'Pubblicato' : 'Bozza'}
            </Badge>
          </div>
        </CardHeader>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <div className="space-y-1">
              ...
              <Button type="button" variant="outline" size="sm" onClick={handleTogglePublish} disabled={isPublishing}>
                ...
                {deckState.isPublished ? 'Rimuovi pubblicazione' : 'Pubblica deck'}
              </Button>
            </div>
          </div>
          <div className="space-y-1">
            ...
            <Textarea
              ...
              onBlur={() => handlePersistDeck()}
              disabled={isSavingDeck}
              placeholder="Contesto, risultati attesi o note di utilizzo..."
      ...
      <Card>
        ...
        <CardContent className="grid gap-3 md:grid-cols-2">
          <div className="space-y-1">
            <label className="text-xs uppercase text-muted-foreground">Fronte</label>
            <Textarea
              value={newCard.front}
          ...
          <div className="space-y-1">
            <label className="text-xs uppercase text-muted-foreground">Retro</label>
            <Textarea
              value={newCard.back}
          ...
          <div className="space-y-1">
            <label className="text-xs uppercase text-muted-foreground">Punti</label>
            <Input
              type="number"
              min={0}
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>ID deck: {deckState.id}</span>
        <span>{deckState.cards.length} carta{deckState.cards.length === 1 ? '' : 'e'}</span>
      </div>
    </div>
  )
}
