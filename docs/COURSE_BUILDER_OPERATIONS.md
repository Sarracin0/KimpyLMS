...
## Gamification Studio (quiz, flashcard & decision lab)
...
- **Generazione**: scegli il tipo di contenuto (quiz, flashcard o decision lab), imposta quantità/difficoltà e premi *Generate with AI*. Il blocco passa dallo stato `DRAFT` → `GENERATING` → `READY` / `FAILED`.
- **Editing**: 
...
- **Decision Lab**: il nuovo toggle genera scenari ramificati in JSON (`GamificationBlock.result.scenario`). Il player learner salva gli attempt in `ScenarioAttempt` con punteggio cumulativo, livello di rischio medio e riflessioni testuali utili agli HR.
...
- **Analytics**: la dashboard `/gamification` riassume tentativi Decision Lab, rischio medio e trend di competenze; le riflessioni più recenti vengono mostrate per facilitare il coaching.
